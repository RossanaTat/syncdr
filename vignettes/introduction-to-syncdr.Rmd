...
##### (+) Comparing directories with `verbose = TRUE`

When using the `compare_directories()` function, you have the option to enable verbose mode by setting `verbose = TRUE`. This will display both directories tree structure and, when comparing files by content, provide progress updates including the time spent hashing the files.
...
or let's display that of files the sync_status of files only in left or right directory
